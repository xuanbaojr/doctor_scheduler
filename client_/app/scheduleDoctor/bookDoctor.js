      ...
      >
        ...
        >
          ...
          <View style={{ flex: 1, marginRight: 16 }}>
            ...
            >
              {propOfDoctor.Specialty.name}
            </Text>
            <View style={{ flexDirection: "row", marginLeft: 10 }}>
              <MaterialIcons name="attach-money" size={15} color="black" />
              <Text
                style={{ fontSize: 13, fontWeight: "800", color: "#6B7280" }}
              >
                {propOfDoctor["price"]}
              </Text>
            </View>
...
const ListDoctor = () => {
  ...
  const getAllDoctor_fn = async () => {
    try {
      const doctors = await instance.get(
        `/doctors_by_specialty/${specialtyId}`
      );
