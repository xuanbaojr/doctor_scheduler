...
const variants = {
  base: 'relative rounded-md aspect-square flex justify-center items-center flex-col cursor-pointer min-h-[100px] min-w-[150px] border border-dashed border-gray-400 dark:border-gray-300 transition-colors duration-200 ease-in-out',
  image:
    'border-0 p-0 w-[50px] h-[50px] relative shadow-md bg-slate-200 dark:bg-slate-900 rounded-md',
  active: 'border-2',
  disabled:
      ...
      <div>
        <div className="flex flex-col ">
          ...
          {(!value || value.length < (dropzoneOptions?.maxFiles ?? 0)) && (
            ...
            >
              ...
              <div className="flex flex-col items-center justify-center text-xs text-gray-400">
                ...
                <div className="mt-3">
                  <Button disabled={disabled}>Gửi</Button>
                </div>
              </div>
            </div>
          )}
          ...
          {value?.map(({ file, progress }, index) => (
            <div
              key={index}
              className={variants.image + ' aspect-square h-full mx-0.5'}
            >
              <img
